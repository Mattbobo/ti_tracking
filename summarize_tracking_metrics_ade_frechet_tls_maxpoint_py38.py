...
def read_prediction_csv(csv_path: Path, subject: str) -> pd.DataFrame:
    ...
    if not required_xy.issubset(df.columns):
        raise ValueError(
            ...
            "無法計算 X、Y、平面定位 ADE、離散弗雷歇距離、對應時間點最大歐氏距離與 TLS。"
        )
...
def trajectory_max_corresponding_distance(group: pd.DataFrame) -> float:
    """
    計算對應時間點最大歐氏距離（Max Corresponding Euclidean Distance），單位為 cm。

    此指標會在相同時間點 / 相同 frame 下比較 Prediction 與 Ground Truth：
    ...
    與離散弗雷歇距離不同，此指標不允許時間彈性對齊，而是嚴格比較同一 frame
    的預測點與真實點，因此可用來觀察即時追蹤情境下的最大瞬時定位誤差。

    當 group 合併多個 csv 時，會分 source_file 分段計算每條軌跡的最大值，
    再取平均，避免不同 csv 檔案之間被錯誤合併。
    """
    values = []
...
def summarize_frame_group(group: pd.DataFrame) -> dict:
    ...
    平面定位 ADE（SD）、離散弗雷歇距離、對應時間點最大歐氏距離、TLS。
    """
    return {
        ...
        "max_corresponding_error_cm": trajectory_max_corresponding_distance(group),
...
def add_formatted_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["對應時間點最大歐氏距離 (cm)"] = out["max_corresponding_error_cm"].map(lambda x: f"{x:.2f}")
...
def main() -> None:
    ...
    with open(report_path, "w", encoding="utf-8-sig") as f:
        ...
        f.write("表格欄位：角度 MAE（SD）、距離 MAE（SD）、X MAE（SD）、Y MAE（SD）、平面定位 ADE（SD）、離散弗雷歇距離、對應時間點最大歐氏距離、軌跡長度相似度 TLS。\n")
        f.write("統計方式：由逐 frame 誤差重新計算；ADE 為平面座標歐氏距離平均；離散弗雷歇距離、對應時間點最大歐氏距離與 TLS 皆以各 csv 檔案分段計算，避免跨檔案相減。\n")
        f.write("補充說明：對應時間點最大歐氏距離係取相同 frame 下 Prediction 與 Ground Truth 之最大平面距離，用於和允許時間順序彈性對齊的離散弗雷歇距離進行比較。\n\n")
