...
# 依照目前 2 RX enabled 設定：
#   NUM_RX = 2             -> 使用 RX0/RX1 做 RDI 與 azimuth
...
# 32 * 2 * 32 * IQ(2) * int16(2) = 8,192 bytes
...
LAST_MODEL_INPUT_PATH = Path("last_model_input_4x64x32_AIC_range_pad2.npy")
...
AZIMUTH_EXTRA_FFTSHIFT = True  # Apply one additional angle-axis fftshift to the azimuth map.
...
AZIMUTH_FOREGROUND_ENABLE = False
...
AZIMUTH_DISPLAY_MAX = 64.0             # 舊版 RDI-near-zero azimuth display scale。raw/range FFT 模式通常不會用到。
AZIMUTH_MIN_ENERGY = 1.0

# ============================================================
# Raw / Range-FFT azimuth feature settings
# ============================================================
# 新版 azimuth 來源：不再從 RDI 的 near-zero Doppler bins 取資料，
# 而是在 raw ADC -> Range FFT 後，直接使用 RX0/RX1 的 complex phase 做 angle FFT。
# 這樣比較不會因為近零速 Doppler bin 每幀跳動而造成 azimuth 閃爍。
AZIMUTH_SOURCE_MODE = "raw_range_fft"   # "raw_range_fft" / "rdi_near_zero"

# 是否在 Range FFT 後、angle FFT 前，沿 chirp 維度扣 mean。
# 訓練資料建議 False，避免慢速/接近靜止的手部訊號被扣掉。
RAW_AZIMUTH_SLOW_TIME_MEAN_REMOVAL = True

# 每個 chirp 都做一次 angle FFT，再沿 chirp 平均。
# "power"：平均 power 後再 sqrt，較穩；"magnitude"：直接平均 magnitude。
RAW_AZIMUTH_CHIRP_AVERAGE_MODE = "power"

# raw/range FFT azimuth 輸出使用 dB，錄製資料與 GUI 顯示使用同一張 map。
# 注意：這不是 per-frame relative dB，不會每幀用自己的最大值重新拉亮。
RAW_AZIMUTH_OUTPUT_DB = True
RAW_AZIMUTH_EPS = 1e-6

# GUI 顯示 raw/range FFT azimuth 時的固定色階，只影響顏色，不改變錄製值。
RAW_AZIMUTH_DISPLAY_VMIN_DB = 55.0
RAW_AZIMUTH_DISPLAY_VMAX_DB = 90.0

# 訓練與顯示都使用同一份 raw/range FFT azimuth，因此預設不做前景增強。
# 如果只為了讓圖更像舊版亮塊，可以改 True，但會同步影響錄製資料。
RAW_AZIMUTH_FOREGROUND_ENABLE = False
RAW_AZIMUTH_SMOOTH_ENABLE = False
...
def crop_rdi_for_model(rdi_complex):
    """
    目前使用 2 RX，所以輸出：
        real/imag split = 4 channels
        model_input.shape = (2, 64, 32)
    """
...
def compute_azimuth_map_from_raw_range_fft(adc_cube):
    """
    Raw / Range-FFT stage azimuth feature.

    Input:
        adc_cube.shape = (chirps, rx, samples)
        建議傳入 AIC 後的 adc_cube_for_rdi，讓 RDI 與 azimuth 使用相同前處理。

    Flow:
        raw ADC
          -> Range FFT
          -> optional slow-time mean removal
          -> per-chirp RX0/RX1 angle FFT
          -> average over chirps
          -> fixed dB map

    Output:
        az_map.shape = (range_bins, angle_bins)
        預設 angle_bins = SAVE_DOPPLER_BINS，因此尺寸會和 RDI 的寬度一致：
        (SAVE_RANGE_BINS, SAVE_DOPPLER_BINS)
    """
    if adc_cube is None or adc_cube.shape != (NUM_CHIRPS_PER_FRAME, NUM_RX, NUM_ADC_SAMPLES):
        raise ValueError(
            f"adc_cube shape mismatch: got {None if adc_cube is None else adc_cube.shape}, "
            f"expected {(NUM_CHIRPS_PER_FRAME, NUM_RX, NUM_ADC_SAMPLES)}"
        )
    if NUM_RX < 2:
        raise ValueError("Need at least two RX channels for raw/range FFT azimuth.")

    # 1) Range FFT along fast-time / ADC sample axis.
    #    這裡不做 Doppler FFT，因此不會依賴 near-zero Doppler bin。
    range_fft = np.fft.fft(adc_cube, n=RANGE_FFT_SIZE, axis=2).astype(np.complex64)

    # 2) Optional slow-time mean removal.
    #    訓練資料預設關閉，因為手部慢速/停住時很容易被扣掉。
    if RAW_AZIMUTH_SLOW_TIME_MEAN_REMOVAL:
        range_fft = range_fft - np.mean(range_fft, axis=0, keepdims=True)

    # 3) Use RX0/RX1 complex phase directly.
    #    ant_cube.shape = (chirps, range_bins, 2)
    ant_cube = np.transpose(range_fft[:, :2, :], (0, 2, 1)).astype(np.complex64, copy=False)

    # 4) Angle FFT for every chirp and range bin.
    angle_bins = max(2, int(AZIMUTH_ANGLE_FFT_BINS))
    az_fft = np.fft.fftshift(
        np.fft.fft(ant_cube, n=angle_bins, axis=2),
        axes=2,
    )

    # 5) Average over chirps.
    avg_mode = str(RAW_AZIMUTH_CHIRP_AVERAGE_MODE).lower()
    if avg_mode == "magnitude":
        angle_mag = np.mean(np.abs(az_fft), axis=0).astype(np.float32)
    else:
        # power average 比單純 coherent sum 穩，不會因為慢速相位轉動互相抵消。
        angle_power = np.mean(np.abs(az_fft) ** 2, axis=0).astype(np.float32)
        angle_mag = np.sqrt(np.maximum(angle_power, 0.0)).astype(np.float32)

    if AZIMUTH_EXTRA_FFTSHIFT:
        angle_mag = np.fft.fftshift(angle_mag, axes=1)

    if AZIMUTH_FLIP_LEFT_RIGHT:
        angle_mag = np.fliplr(angle_mag)

    # 6) Optional feature-domain enhancement. 預設關閉，避免訓練資料變成 display-style 特徵。
    if RAW_AZIMUTH_FOREGROUND_ENABLE:
        angle_mag = enhance_azimuth_foreground(angle_mag)
    if RAW_AZIMUTH_SMOOTH_ENABLE:
        angle_mag = smooth_and_widen_azimuth(angle_mag)

    if RAW_AZIMUTH_OUTPUT_DB:
        az_map = 20.0 * np.log10(np.maximum(angle_mag, float(RAW_AZIMUTH_EPS))).astype(np.float32)
    else:
        az_map = angle_mag.astype(np.float32)

    # 7) Keep same width as RDI for 2-channel recording.
    if az_map.shape[1] != SAVE_DOPPLER_BINS:
        az_map = cv2.resize(
            az_map.astype(np.float32),
            (SAVE_DOPPLER_BINS, az_map.shape[0]),
            interpolation=cv2.INTER_LINEAR,
        )

    return az_map[:SAVE_RANGE_BINS, :SAVE_DOPPLER_BINS].astype(np.float32)
...
def make_two_channel_radar_frame(rdi_complex, azimuth_map=None, display_range_start=None, display_range_end=None):
    """
    產生錄製用的 2-channel radar frame：
        channel 0: 兩個 RX magnitude 平均後的 RDI dB
        channel 1: azimuth feature

    AZIMUTH_SOURCE_MODE = "raw_range_fft" 時，azimuth_map 由 raw ADC / Range FFT 階段產生；
    若沒有傳入 azimuth_map，會 fallback 到舊版 RDI near-zero Doppler 方法。
    ...
    if azimuth_map is None:
        azimuth_map = compute_azimuth_map_from_two_rdi(rdi_complex)
    else:
        azimuth_map = np.asarray(azimuth_map, dtype=np.float32)

    if azimuth_map.shape != rdi_db.shape:
        azimuth_map = cv2.resize(
            azimuth_map.astype(np.float32),
            (rdi_db.shape[1], rdi_db.shape[0]),
            interpolation=cv2.INTER_LINEAR,
        )
    ...
    if display_range_start is not None or display_range_end is not None:
        ...
        azimuth_map = azimuth_map[start:end, :]

    return np.stack([rdi_db.astype(np.float32), azimuth_map.astype(np.float32)], axis=0)
...
class RealtimeRDIViewer:
    ...
    def make_display_images(self, rdi_complex, azimuth_map=None):
        radar_frame = make_two_channel_radar_frame(rdi_complex, azimuth_map=azimuth_map)
    ...
    def update(self, rdi_complex, frame_id, camera_panel=None, azimuth_map=None, force=False):
        if self.closed:
            return None
        ...
        if same_radar_frame and self.last_base_canvas is not None:
            ...
            rdi_db, azimuth_db, radar_frame = self.make_display_images(rdi_complex, azimuth_map=azimuth_map)

        # Azimuth 錄製資料與顯示資料使用同一張 map。
        # raw_range_fft 模式下，azimuth_db 是固定 dB map；這裡的 vmin/vmax 只影響色彩顯示。
        if str(AZIMUTH_SOURCE_MODE).lower() == "raw_range_fft":
            alpha = float(AZIMUTH_TEMPORAL_EMA_ALPHA)
            alpha = max(0.0, min(1.0, alpha))
            if AZIMUTH_DENOISE_ENABLE and alpha > 0.0:
            ...
            az_vmin = float(RAW_AZIMUTH_DISPLAY_VMIN_DB)
            az_vmax = float(RAW_AZIMUTH_DISPLAY_VMAX_DB)
            az_unit = " dB" if RAW_AZIMUTH_OUTPUT_DB else ""
        elif AZIMUTH_DENOISE_ENABLE:
            alpha = float(AZIMUTH_TEMPORAL_EMA_ALPHA)
            alpha = max(0.0, min(1.0, alpha))
            if self.azimuth_ema is None or self.azimuth_ema.shape != azimuth_db.shape:
                self.azimuth_ema = azimuth_db.astype(np.float32).copy()
        ...
        az_tile = self._make_heatmap_tile(
            azimuth_db,
            "Azimuth",
            "raw Range-FFT RX angle FFT" if str(AZIMUTH_SOURCE_MODE).lower() == "raw_range_fft" else "near-zero Doppler angle FFT",
            az_vmin,
            az_vmax,
            az_unit,
        )
        ...
        title = (
            ...
            f"az source={AZIMUTH_SOURCE_MODE} | "
...
class DCA1000UDPReceiver:
    ...
    def get_latest_rdi(self):
        """
        回傳目前最新一幀 complex RDI 與 azimuth map 的 copy，避免 GUI 顯示時剛好被接收執行緒改到。
        """
        with self.rdi_lock:
            if self.latest_rdi_complex is None:
                return None, -1, None

            az_copy = None if self.latest_azimuth_map is None else self.latest_azimuth_map.copy()
            return self.latest_rdi_complex.copy(), int(self.latest_rdi_frame_id), az_copy
    ...
    def _loop(self):
        ...
        print(f"Azimuth source: {AZIMUTH_SOURCE_MODE}")
        if str(AZIMUTH_SOURCE_MODE).lower() == "raw_range_fft":
            print(f"Azimuth raw : Range FFT -> per-chirp RX angle FFT -> {RAW_AZIMUTH_CHIRP_AVERAGE_MODE} average")
        elif AZIMUTH_USE_NEAR_ZERO_DOPPLER:
        ...
        try:
            while self.running:
                ...
                while len(self.raw_buffer) >= FRAME_BYTES:
                    ...
                    if str(AZIMUTH_SOURCE_MODE).lower() == "raw_range_fft":
                        azimuth_map = compute_azimuth_map_from_raw_range_fft(adc_cube_for_rdi)
                    else:
                        azimuth_map = None
                    ...
                    # 即時顯示/錄製用：只保存最新一幀的 complex RDI 與 azimuth map。
                    with self.rdi_lock:
                        self.latest_rdi_complex = rdi_complex.copy()
                        self.latest_azimuth_map = None if azimuth_map is None else azimuth_map.copy()
    ...
    try:
        ...
        while True:
            ...
            if viewer is not None and not viewer.closed:
                latest_rdi, latest_frame_id, latest_azimuth_map = receiver.get_latest_rdi()
                if latest_rdi is not None:
                    maybe_frame = viewer.update(latest_rdi, latest_frame_id, camera_panel=camera_panel, azimuth_map=latest_azimuth_map)
