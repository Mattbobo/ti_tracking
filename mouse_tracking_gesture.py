...
def ask_gesture(seq: np.ndarray):
    """Send gesture request, return 1 when a gesture is detected."""
    sock.send(pickle.dumps(("gesture", seq), protocol=4))
    return sock.recv_pyobj()


def load_gesture_mean_std(path):
    with open(path, "r", encoding="utf-8") as f:
        cfg = json.load(f)

    mean = np.asarray(cfg.get("mean"), dtype=np.float32)
    std = np.asarray(cfg.get("std"), dtype=np.float32)
    if mean.shape != (2,) or std.shape != (2,):
        raise RuntimeError(f"Invalid gesture mean/std file: {path}")

    std = np.maximum(std, 1e-6)
    return mean.reshape(1, 2, 1, 1), std.reshape(1, 2, 1, 1)
...
class ShowDataView(QtWidgets.QWidget):
    def __init__(self, args):
        ...
        self.gesture_mean, self.gesture_std = (
            load_gesture_mean_std(self.args.gesture_mean_std)
            if self.args.enable_gesture
            else (None, None)
        )
        ...
        self.gesture_hold_ms = max(1, int(self.args.gesture_hold_ms))
        self.gesture_timer = QtCore.QElapsedTimer()
        self.gesture_freeze_local = None
        self.pred_color = QtGui.QColor(255, 80, 80)
        self.gesture_color = QtGui.QColor(80, 255, 80)
    ...
    def _build_ui(self):
        ...
        self.gesture_label.setFont(QtGui.QFont("", 12))
        ...
        hl4.addWidget(self.gesture_label)
        hl4.addStretch()
    ...
    def _current_display_local(self):
        if self.pred_display_local is not None:
            return np.asarray(self.pred_display_local, dtype=np.float32).copy()
        if self.pred_hold_local is not None:
            return np.asarray(self.pred_hold_local, dtype=np.float32).copy()
        if self.pred_target_local is not None:
            return np.asarray(self.pred_target_local, dtype=np.float32).copy()
        return None

    def _start_gesture_hold(self):
        self.gesture_active = True
        self.gesture_timer.start()
        self.gesture_freeze_local = self._current_display_local()

    def _show_gesture_hold(self):
        self.pred_hold_active = False
        self.pred_angle = None
        self.pred_dist = None
        self.pred_label.setText("Predicted: Gesture Triggered")
        self.gesture_label.setText("Gesture: Triggered")

        if self.gesture_freeze_local is not None:
            gx, gy = self.gesture_freeze_local
            self.pred_xy_label.setText(
                f"Pred Grid XY: {gx / self.grid_cm:.1f}, {gy / self.grid_cm:.1f} (cells)"
            )
        else:
            self.pred_xy_label.setText("Pred Grid XY: --, -- (cells)")
    ...
    def updateRadar(self, packet):
        ...
        if self.args.enable_gesture:
            seq_gesture = (seq - self.gesture_mean) / self.gesture_std
            gesture_detected = bool(ask_gesture(seq_gesture))

            if gesture_detected and not self.gesture_active:
                self._start_gesture_hold()

            if self.gesture_active:
                if self.gesture_timer.elapsed() < self.gesture_hold_ms:
                    self._show_gesture_hold()
                    return
                self._finish_gesture_hold()
    ...
    def updateXY(self):
        ...
        if self.gesture_active and self.gesture_freeze_local is not None:
            gx, gy = self.gesture_freeze_local
            if self._inside_pad_loose(float(gx), float(gy)):
                draw_local_point(gx, gy, self.gesture_color)
        elif self.pred_display_local is not None and (self.pred_angle is not None or self.pred_hold_active):
            dx, dy = self.pred_display_local
            if self._inside_pad_loose(float(dx), float(dy)):
                draw_local_point(dx, dy, self.pred_color)
...
def build_arg_parser():
    ...
    parser.add_argument("--gesture-mean-std", type=str, default=r"gesture_file\mean_std.json")
    parser.add_argument("--seq-len", type=int, default=20)
    parser.add_argument("--enable-gesture", action="store_true", default=True)
    parser.add_argument("--no-gesture", action="store_false", dest="enable_gesture")
    parser.add_argument("--gesture-hold-ms", type=int, default=1500)
